 ...
 * @param {FormData | Record<string, unknown>} inputData - Les données brutes du formulaire (ou un objet JS).
 * @param {FormDataProcessingOptions<T>} [options={}] - Options pour contrôler le pipeline de traitement.
 * @returns {ProcessedFormDataResult<T>} Un objet résultat indiquant succès/échec, les données formatées en cas de succès,
 * ou les erreurs et les données de débogage en cas d'échec.
 *
 * @example
...
export function processAndValidateFormData<T extends z.ZodRawShape>(
  ...
  const {
    ...
    transformations = {},
    outputFormat = 'object',
    additionalData = {}, // Initialise à un objet vide si non fourni
    ...extractOptions
  } = options;
  ...
  const validationResult = useDynamicValidation
    ? validateDataWithDynamicSchema(schema, dataToProcess)
    : schema.safeParse(dataToProcess);
  ...
  if (validationResult.success) {
    ...
    // Échec de la validation : retourner les erreurs et les données brutes/transformées pour débogage/affichage
    return {
      success: false,
      data: dataToProcess, // Retourne les données après fusion et transformations pour le débogage
      errors: formatZodErrorsAsObject(validationResult),
      errorsInArray: formatZodErrorsAsArray(validationResult),
    };
  }
}
